...
// ── Rank tier ─────────────────────────────────────────────────────────────
function rankTier(level: number): {
  label: string;
  color: string;
  icon: string;
} {
...
// ── Pure geometry ─────────────────────────────────────────────────────────
function screenY(worldY: number, camY: number) {
  return H - (worldY - camY);
}
function platWY(level: number) {
  return GROUND_Y + level * PLATFORM_SPACING;
}
...
function platW(level: number) {
  return level === 0 ? W - 20 : Math.max(40, 112 - level * 0.6);
}
function platCol(level: number) {
  return PLAT_COLORS[level % PLAT_COLORS.length];
}
function zoneIdx(level: number) {
  return Math.floor(level / ZONE_SIZE) % ZONE_GRADIENTS.length;
}

// ── Audio ─────────────────────────────────────────────────────────────────
let _actx: AudioContext | null = null;
function actx() {
  return (_actx ??= new AudioContext());
}
function tone(
  freq: number,
  dur: number,
...
const sndDie = () => {
  tone(220, 0.18, 0.2, "sawtooth");
  setTimeout(() => tone(160, 0.22, 0.14, "sawtooth"), 80);
};
const sndWin = () =>
  [523, 659, 784, 1047].forEach((f, i) =>
    setTimeout(() => tone(f, 0.18, 0.18), i * 90),
  );
...
function drawBg(ctx: CanvasRenderingContext2D, camY: number, level: number) {
  ...
  if (level < 50 || zoneIdx(level) <= 1) {
    ...
    for (let i = 0; i < 40; i++) {
      ctx.fillRect(
        ((seed * 137 + i * 73) & 0xffff) % W,
        ((seed * 53 + i * 29) & 0xffff) % H,
        1.5,
        1.5,
      );
    }
...
function drawGround(ctx: CanvasRenderingContext2D, camY: number) {
  ...
  for (const h of houses) {
    ...
    ctx.fillRect(dx, gy - dh, dw, dh);
    ctx.fillStyle = "#fbbf24";
    ctx.beginPath();
    ctx.arc(dx + dw - 7, gy - dh / 2, 3, 0, Math.PI * 2);
    ctx.fill();
    ...
    for (const bx of [h.x + 5, h.x + h.w - 30]) {
      ctx.beginPath();
      ctx.arc(bx + 12, gy, 14, 0, Math.PI * 2);
      ctx.fill();
    }
  }
}
...
function drawPlat(
  ...
  const cx = platCX(level, elapsed),
    pw = platW(level);
  const sy = screenY(platWY(level), camY);
  if (sy < -PLAT_H - 4 || sy > H + 4) return;
  ctx.fillStyle = platCol(level);
  ctx.beginPath();
  ctx.roundRect(cx - pw / 2, sy, pw, PLAT_H, 5);
  ctx.fill();
  ctx.fillStyle = "rgba(255,255,255,0.18)";
  ctx.beginPath();
  ctx.roundRect(cx - pw / 2, sy, pw, 4, [5, 5, 0, 0]);
  ctx.fill();
}
...
function drawEgg(
  ...
  ctx.beginPath();
  ctx.ellipse(x, sy + EGG_R + 3, EGG_R * 0.72, 5, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = color;
  ctx.beginPath();
  ctx.ellipse(x, sy, EGG_R * 0.85, EGG_R, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(255,255,255,0.5)";
  ctx.beginPath();
  ctx.ellipse(
  ...
  ctx.fillStyle = "#fff";
  ctx.fillText(label, x, sy - EGG_R - 5);
  ctx.restore();
}
...
// ── Component ─────────────────────────────────────────────────────────────
export default function LemparTelur({
  roomId,
  userId,
  username,
  pigColor,
  ...
  const soloScoreSavedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    socket.on(
      "egg_game_start",
      ({ players, startTime }: { players: SrvPlayer[]; startTime: number }) => {
        gs.players = players;
    ...
    socket.on(
      ...
      ({ id, level }: { id: string; level: number }) => {
        ...
        if (o) {
          o.state = "resting";
          o.vy = 0;
          o.platformLevel = level;
          o.worldY = platWY(level);
        }
      },
    );
    socket.on(
    ...
    socket.on(
      "egg_player_died",
      ({ id, players }: { id: string; players: SrvPlayer[] }) => {
        gs.players = players;
        const o = gs.others.get(id);
        if (o) o.state = "dead";
        if (id === myIdRef.current && gs.phase === "playing") {
          gs.myState = "dead";
          gs.phase = "spectating";
          sndDie();
        }
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      ...
      if (e.code === "KeyR" && gs.phase === "gameover" && gs.solo)
        restartSolo();
    };
    const up = () => {
      gs.jumpPressed = false;
  ...
  function restartSolo() {
    soloScoreSavedRef.current = false;
    gs.phase = "playing";
    gs.worldY = 0;
    gs.vy = 0;
    gs.myState = "resting";
    ...
    function loop() {
      ...
      if (gs.phase === "playing") {
        if (gs.jumpPressed && !gs.jumpConsumed && gs.myState === "resting") {
          ...
          sndJump();
          if (!gs.solo)
            socket?.emit("egg_jump", { fromLevel: gs.platformLevel });
        }
        ...
        if (gs.myState === "jumping") {
          ...
          if (gs.vy < 0) {
            let landed = false;
            for (
              let lv = gs.platformLevel + 1;
              lv <= gs.platformLevel + 12 && !landed;
              lv++
            ) {
              const pWY = platWY(lv);
      ...
      for (const o of gs.others.values()) {
        if (o.state === "jumping") {
          ...
          o.x = platCX(o.platformLevel, elapsed);
          o.worldY = platWY(o.platformLevel);
        }
      }
      ...
      if (gs.myState !== "dead") {
        ...
        let highY = -Infinity;
        for (const o of gs.others.values())
          if (o.state !== "dead") highY = Math.max(highY, o.worldY);
      ...
      for (const o of gs.others.values())
        visLv = Math.max(visLv, o.platformLevel);
      ...
      for (let lv = botLv; lv <= topLv; lv++)
        drawPlat(ctx, lv, elapsed, gs.camY);

      for (const o of gs.others.values())
        drawEgg(
          ctx,
          o.x,
          screenY(o.worldY, gs.camY) - EGG_R,
          EGG_COLORS[o.slot] ?? "#fff",
          o.username,
          o.state === "dead",
        );
      ...
      if (gs.phase !== "lobby")
        drawEgg(
          ctx,
          gs.x,
          screenY(gs.worldY, gs.camY) - EGG_R,
          EGG_COLORS[gs.mySlot] ?? pigColor,
          username,
          gs.myState === "dead",
        );
      ...
      if (gs.phase === "lobby") {
        ...
        if (gs.host === myId) {
          if (gs.players.length >= 2) {
            ctx.fillStyle = "#4ade80";
            ctx.beginPath();
            ctx.roundRect(W / 2 - 85, H - 115, 170, 46, 10);
            ctx.fill();
            ctx.fillStyle = "#000";
      ...
      if (gs.phase === "gameover") {
        ...
        if (gs.solo) {
          ...
          if (!soloScoreSavedRef.current) {
            soloScoreSavedRef.current = true;
            fetch("/api/score", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ userId, score, gameKey: "egg_solo" }),
            });
          }
          ...
          tiers.forEach((t, i) => {
            const active =
              gs.highestLevel >= t.min &&
              (i === tiers.length - 1 || gs.highestLevel < tiers[i + 1].min);
          ...
          ctx.fillStyle = "#4ade80";
          ctx.beginPath();
          ctx.roundRect(W / 2 - 90, H - 108, 180, 44, 10);
          ctx.fill();
          ctx.fillStyle = "#000";
          ...
          const votes = gs.rematchVotes,
            total = gs.rematchTotal || gs.players.length;
          ctx.fillStyle = "#86efac";
          ctx.beginPath();
          ctx.roundRect(W / 2 - 95, H - 108, 190, 44, 10);
          ctx.fill();
          ctx.fillStyle = "#000";
          ctx.font = "bold 17px sans-serif";
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <p className="text-white/60 text-xs">
        Tekan <kbd className="bg-white/20 px-1 rounded">Spasi</kbd> /{" "}
        <kbd className="bg-white/20 px-1 rounded">↑</kbd> atau tap untuk loncat
        {isSolo && (
          <>
            {" "}
            · <kbd className="bg-white/20 px-1 rounded">R</kbd> untuk restart
          </>
        )}
      </p>
    </div>
